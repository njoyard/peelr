...
export default class PeelrContext extends EventEmitter {
  ...
  async getHTML() {
    ...
    if (isRequestParams(source)) {
      ...
      if (
        ...
        let debounceKey = new URL(source.url || source.uri).hostname

        req = cache[cacheKey] = debounce(() => {
          this.emit('request', source, false)
          return request(Object.assign({ jar }, defaultRequestParams, source))
        }, debounceKey)
      }
  ...
  async derive(params) {
    ...
    derived.on('extract', (url, selector, data) =>
      this.emit('extract', url, selector, data)
    )
